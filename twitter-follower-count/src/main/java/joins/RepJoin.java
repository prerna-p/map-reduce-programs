...
public class RepJoin {
  ...
  public static class RepJoinMapper extends Mapper<Object, Text, NullWritable, NullWritable> {
    ...
    public void setup(Context context) throws IOException, InterruptedException {
      try {
        ...
        if (files == null || files.length == 0) {
          throw new RuntimeException("User information is not set");
        }

        for (FileStatus f : files) {
          BufferedReader rdr = new BufferedReader(new InputStreamReader(fs.open(f.getPath())));
    ...
    public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
      ...
      /*
       * for each input node pair, iterate through the child(ren) of the "to" node and
       * find occurence of the to-parent
       */
