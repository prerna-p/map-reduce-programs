...
object RDDG {
  ...
  def main(args: Array[String]) {
    ...
    // setup logging
    val log = Logger.getLogger(RDDG.getClass())

    // report error if input or ouput are not specified
    if (args.length != 2) {
      log.error("Usage:\ntfcount.RDDG <input dir> <output dir>")
      System.exit(1)
    }

    // set spark context
    val conf = new SparkConf().setAppName("Twitter Followers Count").setMaster("local")
     ...
     * group all the user-ids with corresponding follow counts
     * sum up the follow counts
     */
